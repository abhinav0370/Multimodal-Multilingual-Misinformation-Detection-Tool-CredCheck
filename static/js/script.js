...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function displayLiveBroadcastAnalysis(data) {
        ...
        if (data.results && data.results.length > 0) {
            ...
            const displayedProcessingMessages = new Set();

            sortedResults.forEach(result => {
                // Clean any LaTeX formatting
                const cleanedNews = cleanLatexFormatting(result.news);

                // Skip processing messages that we've seen before
                if (cleanedNews.startsWith("Processing video:")) {
                    // Extract video title from the message
                    const videoTitle = cleanedNews.replace("Processing video:", "").trim();
                    if (displayedProcessingMessages.has(videoTitle)) {
                        return; // Skip duplicate processing messages
                    }
                    displayedProcessingMessages.add(videoTitle);

                    // Also check if we already have real analysis for this video
                    // If we do, skip showing the processing message
                    for (const r of sortedResults) {
                        const newsText = cleanLatexFormatting(r.news);
                        if (!newsText.startsWith("Processing video:") &&
                            newsText !== cleanedNews) {
                            // This is a real analysis, so skip the processing message
                            return;
                        }
                    }
                }
                // For regular news items
                else if (displayedNews.has(cleanedNews)) {
                    return; // Skip duplicate news
                }

                displayedNews.add(cleanedNews);
