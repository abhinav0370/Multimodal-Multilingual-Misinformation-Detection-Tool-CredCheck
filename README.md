...
For high-volume processing and improved scalability, CredCheck now supports Apache Kafka integration.
...
3. Access the Kafdrop UI to monitor Kafka topics:
   ```
   http://localhost:9000/
   ```

4. Kafka topics used by the application:
   - `raw-content`: Contains YouTube URLs and news articles for processing
   - `analysis-results`: Contains processed analysis results

5. To run the application with Kafka integration, make sure these environment variables are set in your `.env` file:
   ```
...
- Decoupled processing pipeline for better scalability
- Fault tolerance - analysis continues even if web app restarts
- Improved performance for processing multiple news sources simultaneously
- Real-time processing of live broadcasts
